...
type
  ...
  private
    ...
    procedure House_MarketTargetHighlight(Sender: TObject);
...
begin
  ...
  else if aMarket.TransferTo <> nil then
  begin
    Label_Market_Out.Caption := Format(gResTexts[TX_HOUSES_MARKET_TO], [0]);
    Button_Market_Out.TexID := gRes.Houses[htStore].GUIIcon;
    Button_Market_Out.FlagColor := gHands[aMarket.TransferTo.Owner].FlagColor;
    Button_Market_Out.Clickable := True;
    Button_Market_Out.OnClick := House_MarketTargetHighlight;
  ...
  Button_Market_Remove.Enabled := (aMarket.ResFrom <> wtNone) and ((aMarket.ResTo <> wtNone) or (aMarket.TransferTo <> nil));
...
procedure TKMGUIGameHouse.House_MarketTargetHighlight(Sender: TObject);
var
  H: TKMHouseStore;
begin
  if not (gMySpectator.Selected is TKMHouseMarket) then Exit;

  H := TKMHouseMarket(gMySpectator.Selected).TransferTo;
